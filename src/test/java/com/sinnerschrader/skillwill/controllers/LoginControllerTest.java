...
public class LoginControllerTest {
	...
	public void testLogoutValid() throws JSONException {
		...
		assertTrue(res.getStatusCode() == HttpStatus.OK);
		JSONObject resJSON = new JSONObject(res.getBody());
		assertTrue(loginController.logout("foobar", resJSON.getString("session")).getStatusCode() == HttpStatus.OK);
	}
	...
	public void testLogoutInvalidSession() {
		ResponseEntity<String> res = loginController.login("foobar", "fleischcreme");
		assertTrue(res.getStatusCode() == HttpStatus.OK);
		assertTrue(loginController.logout("foobar", "NotQuiteASessionId").getStatusCode() == HttpStatus.BAD_REQUEST);
	}

	@Test
	public void testLogoutInvalidUser() throws JSONException {
		ResponseEntity<String> res = loginController.login("foobar", "fleischcreme");
		assertTrue(res.getStatusCode() == HttpStatus.OK);
		JSONObject resJSON = new JSONObject(res.getBody());
		assertTrue(loginController.logout("NotAUser", resJSON.getString("session")).getStatusCode() == HttpStatus.BAD_REQUEST);
	}
